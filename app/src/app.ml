...
let one_segment_node segment =
  let open Vdom in
  Node.tr ~attrs:[ Attr.class_ "mdc-data-table__row" ]
  @@ List.map
       ~f:(fun elem ->
         Node.td ~attrs:[ Attr.class_ "mdc-data-table__cell" ] [ Node.text elem ])
       segment
...
let runtime_node profile =
  ...
  Node.div
    ~attrs:[ Attr.create "style" "display: flex; justify-content: center" ]
    [
      Node.div
        ~attrs:[ Attr.class_ "mdc-data-table"; Attr.id "runtime-table" ]
        [
          Node.div
            ~attrs:[ Attr.class_ "mdc-data-table__table-container" ]
            [
              Node.table
                ~attrs:[ Attr.class_ "mdc-data-table__table" ]
                [
                  Node.thead
                    [
                      Node.tr
                        ~attrs:[ Attr.class_ "mdc-data-table__header-row" ]
                        [
                          header_cell "";
                          header_cell "Depth";
                          header_cell "Stop";
                  ...
                  Node.tbody ~attrs:[ Attr.class_ "mdc-data-table__content" ]
...
let one_tank_supply_node gas_supply tank =
  ...
  Node.li
    ~attrs:[ Attr.class_ "mdc-list-item" ]
    [
      Node.span ~attrs:[ Attr.class_ "mdc-list-item__ripple" ] [];
      Node.span
        ~attrs:[ Attr.class_ "mdc-list-item__text" ]
        [
          Node.span
            ~attrs:[ Attr.class_ "mdc-list-item__primary-text" ]
            [
              Node.text
              @@ Fmt.str "%.0f bar of %a" remaining_pressure Gas.pp (Tank.gas tank);
            ];
          Node.span
            ~attrs:[ Attr.class_ "mdc-list-item__secondary-text" ]
            [
              Node.text
              @@ Fmt.str
                   "%.0f L in %a L tank"
                   ...
                   (Fmt.float_dfrac 1)
                   (Physics.to_litre @@ Tank.volume tank);
            ];
        ];
    ]
...
let gas_supply_node gas_supply =
  ...
  Node.section
    [
      Node.h2
        ~attrs:[ Attr.class_ "mdc-typography--headline5" ]
        [ Node.text "Remaining gas" ];
      Node.ul
        ~attrs:[ Attr.classes [ "mdc-list"; "mdc-list--two-line" ] ]
...
let view m ~inject =
  ...
  let input name action =
    ...
    Node.label
      ...
      [
        Node.span ~attrs:[ Attr.class_ "mdc-text-field__ripple" ] [];
        Node.input
          ~attrs:
            [
              Attr.id input_id;
              ...
              Attr.on_input (fun _ev text -> inject (action text));
            ]
          ();
        Node.span
          ~attrs:[ Attr.class_ "mdc-floating-label"; Attr.id label_id ]
          [ Node.text name ];
        Node.span ~attrs:[ Attr.class_ "mdc-line-ripple" ] [];
      ]
  in
  let depth_input =
    input "Depth" (fun text -> Action.Update_depth (float_of_string text))
  in
  let time_input =
    input "Time" (fun text ->
  ...
  let%map full_profile =
    ...
    let profile = Profile.square Param.default ~tank:bottom_tank ~depth ~time in
    let gf = (0.8, 0.8) in
    let deco = Buhlmann.deco_procedure Param.default gf tanks profile in
    Profile.append profile deco
  in
  let runtime_node = runtime_node full_profile in
  let gas_supply =
    Gas_supply.breathe_on_profile Param.default full_profile gas_supply_full
  in
  let gas_supply_node = gas_supply_node gas_supply in
  Node.div
    [
      Node.div
        ~attrs:[ Attr.class_ "mdc-layout-grid" ]
        [
          Node.div
            ~attrs:[ Attr.class_ "mdc-layout-grid__inner" ]
...
let create model ~old_model:_ ~inject =
  let open Incr.Let_syntax in
  let%map apply_action = model >>| apply_action
