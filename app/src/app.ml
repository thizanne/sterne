...
let one_segment_node segment =
  ...
  List.map
    ~f:(fun elem -> Node.td ~attrs:[Attr.class_ "mdc-data-table__cell"] [Node.text elem])
    segment
...
let runtime_node profile =
  ...
  Node.div
    ...
    [
      Node.div ~attrs:[Attr.class_ "mdc-data-table"; Attr.id "runtime-table"] [
        Node.div ~attrs:[Attr.class_ "mdc-data-table__table-container"] [
          Node.table ~attrs:[Attr.class_ "mdc-data-table__table"] [
            Node.thead [
              Node.tr ~attrs:[Attr.class_ "mdc-data-table__header-row"] [
                header_cell "";
                header_cell "Depth";
...
let one_tank_supply_node gas_supply tank =
  ...
  Node.li ~attrs:[Attr.class_"mdc-list-item"] [
    Node.span ~attrs:[Attr.class_ "mdc-list-item__ripple"] [];
    Node.span ~attrs:[Attr.class_ "mdc-list-item__text"] [
      Node.span ~attrs:[Attr.class_ "mdc-list-item__primary-text"] [
        Node.text @@
        Fmt.str "%.0f bar of %a"
          remaining_pressure
          Gas.pp (Tank.gas tank)
      ];
      Node.span ~attrs:[Attr.class_ "mdc-list-item__secondary-text"] [
        Node.text @@
...
let gas_supply_node gas_supply =
  let open Vdom in
  Node.section [
    Node.h2 ~attrs:[Attr.class_ "mdc-typography--headline5"] [Node.text "Remaining gas"];
    Node.ul ~attrs:[Attr.classes ["mdc-list"; "mdc-list--two-line"]] (
...
let view m ~inject =
  ...
  let time_input =
    input
      "Time"
      (fun text -> Action.Update_time (Time_float.Span.of_min @@ float_of_string text)) in
  let mdc_cell elt =
