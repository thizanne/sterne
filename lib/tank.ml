...
module T = struct
  ...
  let compare { id = x; _ } { id = y; _ } = Id.compare x y
end
...
let find_best ~ppo2_max ~depth available_tanks =
  (* TODO: move the comparison logic in Gas, and consider returning the less bad gas if
     none can be breathed. Also consider the empty list. And maybe a balancing option,
     switching from the current tank only if one is found with more pressure. *)
  let compare_tank_gasses { gas = gas1; _ } { gas = gas2; _ } =
    (* Negative => tank1 is better. We use lists to lexicographically compare gasses: more
...
let find_best_deco param = find_best ~ppo2_max:param#ppo2_max_deco
let find_best_bottom param = find_best ~ppo2_max:param#ppo2_max_bottom

(** Common tanks *)

type tank_creator = Gas.t -> unit -> t

let tank_creator vol_litre start_pressure gas () =
